...
### Native builds (needed for deep links / password reset)

Expo Go can't register the app's custom `staffscheduling://` URL scheme, so
testing the forgot-password deep link requires a real native build
(`npx expo run:android` / `npx expo run:ios`) instead. A few things to know
if you're setting this up fresh:

- **Use JDK 17 or 21 for Android, not whatever's newest.** A very new JDK
  (e.g. 25) breaks the Android Gradle Plugin's native/CMake build step with
  an opaque `WARNING: A restricted method in java.lang.System has been
  called` failure. Install one with `brew install openjdk@21` and point
  `JAVA_HOME` at it before building.
- If `./gradlew` can't find the SDK, make sure `ANDROID_HOME` is set and
  `android/local.properties` has `sdk.dir=<path-to-sdk>` — `expo run:android`
  normally does this for you, but running `gradlew` directly does not.
- Android deep links while the app is already running (not a cold start)
  need `MainActivity.onNewIntent` to call `setIntent(intent)`, otherwise
  `Linking`'s `"url"` event never fires. Since `android/` is gitignored and
  regenerated by `expo prebuild`, this is added by a config plugin
  (`plugins/withAndroidOnNewIntent.js`, registered in `app.json`) rather
  than hand-edited, so it survives a clean prebuild.
- Verified end-to-end on Android: forgot-password → reset email → tapping
  the `staffscheduling://reset-password#...` link reopens the app straight
  to the reset-password screen, including while the app was already open.
...
- Forgot password (login screen) and change password (Profile screen, both
  ...
  The deep link itself (tapping the emailed reset link reopening the app to
  the reset-password screen) is verified end-to-end on Android — see
  [Native builds](#native-builds-needed-for-deep-links--password-reset)
  above for what that build needs.
